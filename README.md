...
Dem Repository liegt mit `dashboard.yaml` ein fertiges Beispiel-Dashboard für die
JUDO i-soft PRO / L Integration bei.

Die Datei befindet sich **direkt im Hauptverzeichnis des GitHub-Repositories**, also
auf derselben Ebene wie `README.md`, `LICENSE` und `hacs.json`.

## Dashboard aktivieren

Das Dashboard wird durch HACS nicht automatisch als eigenes Home Assistant Dashboard
aktiviert. Nach der Installation der Integration kann die mitgelieferte
`dashboard.yaml` jedoch einfach als Grundlage für ein eigenes Dashboard verwendet
werden.

### Variante 1: Über die Home Assistant Oberfläche

1. Öffnen Sie **Einstellungen → Dashboards**.
2. Erstellen Sie über **Dashboard hinzufügen** ein neues Dashboard.
3. Öffnen Sie im neuen Dashboard das Menü **⋮**.
4. Öffnen Sie den **Rohkonfigurationseditor**.
5. Öffnen Sie die Datei `dashboard.yaml` aus diesem Repository.
6. Kopieren Sie den vollständigen Inhalt der Datei.
7. Fügen Sie den Inhalt in den Rohkonfigurationseditor ein.
8. Speichern Sie die Konfiguration.

Die mitgelieferte Datei enthält bereits die Ansicht **„i-soft PRO“** mit Bereichen
für Wasserverbrauch, Salzvorrat sowie die Steuerung der Anlage.
...
### Variante 2: Als YAML-Dashboard einbinden

Fortgeschrittene Benutzer können `dashboard.yaml` auch als separates YAML-Dashboard
einbinden.

Dazu kann die Datei in das `/config`-Verzeichnis von Home Assistant kopiert und
anschließend in der Dashboard-Konfiguration eingebunden werden.
...
lovelace:
  dashboards:
    judo_isoft:
      mode: yaml
      title: JUDO i-soft PRO
      icon: mdi:water-softener
      filename: dashboard.yaml
      show_in_sidebar: true
```

Die Datei `dashboard.yaml` muss dabei im `/config`-Verzeichnis liegen.

> **Hinweis:** Das Dashboard ist eine Vorlage und kann jederzeit an die eigene
> Home Assistant Installation angepasst und erweitert werden.
